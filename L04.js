...
function test1() {
	...
	// Skapar och tilldelar variabeln tal
	var tal = 10;
...
function test6(text) {
	...
	var scndlastChar = text.charAt(text.length-2);
	return scndlastChar;
...
function test8(words) {
	...
	var last_word = words.substr(words.lastIndexOf(" ") + 1);
	return last_word;
}
...
// annars ska ytterligare en test göras ifall variablen epost innehåller ett @-tecken. 
// Är det sant ska texten "Ditt meddelande skickas" returneras
...
function test11(arr) {
	...
	var middle_value = arr[Math.round(arr.length -1) / 2];
